...
class FirestoreService {
  ...
  Future<void> updatePhotos(String uid, List<String> imageUrls) async {
    try {
      ...
      debugPrint("updatePhotos error: $e");
    }
  }
  ...
  Stream<List<MotoSosAlert>> streamActiveSosAlerts() {
    return _sosRef
        ...
        .snapshots()
        .map((snapshot) {
      return snapshot.docs.map((doc) => MotoSosAlert.fromFirestore(doc)).toList();
  ...
  List<RiderBadge> getUserBadges(MotoUser user) {
    return [
      ...
      RiderBadge(
        ...
        category: "Performans",
        isUnlocked: user.ridingStyle.contains("Racing") || user.isVerified,
        currentProgress: 380,
        targetProgress: 500,
      ),
      RiderBadge(
      ...
      RiderBadge(
        ...
        category: "Macera",
        isUnlocked: user.ridingStyle.contains("Enduro"),
        currentProgress: user.ridingStyle.contains("Enduro") ? 3 : 1,
        targetProgress: 3,
      ),
      RiderBadge(
        id: 'badge_vip',
  ...
  Stream<List<LeaderboardEntry>> streamRealLeaderboardEntries(MotoUser currentUser) {
    return _usersRef.snapshots().map((snapshot) {
      ...
      for (int i = 0; i < realUsers.length; i++) {
        ...
        final weeklyKm = totalLean > 0 ? (totalLean * 6 + 50).toInt() : (120 - i * 10).clamp(10, 500);
        entries.add(
          LeaderboardEntry(
            rank: i + 1,
            rider: user,
  ...
  Stream<List<RideEvent>> getRidesStream() {
    return _ridesRef.snapshots().map((snapshot) {
      return snapshot.docs.map((doc) => RideEvent.fromFirestore(doc)).toList();
