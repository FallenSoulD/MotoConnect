...
class AuthService {
  ...
  Future<MotoUser?> signInWithApple() async {
    try {
      ...
      // Mükerrer veya sahte profilleri temizle
      if (email.isNotEmpty) {
        await FirestoreService().cleanDuplicateUserProfiles(firebaseUser.uid, email);
      }
      ...
      if (userProfile == null) {
        ...
        // Eğer profil mevcutsa ve kullanıcı adı güncellenebiliyorsa
        if (nickname.isNotEmpty && nickname != "Apple Sürücüsü" && (userProfile.nickname == "Apple Sürücüsü" || userProfile.nickname.isEmpty)) {
      ...
      if (e.code == AuthorizationErrorCode.canceled || e.code == AuthorizationErrorCode.unknown) {
        // Kullanıcı Face ID veya Apple diyaloğunu iptal etti
        return null;
      }
      rethrow;
